...
class Firemaking : Script {
    init {
        itemOnObject("Light", "logs_*", "*") {
            val level = player["skill_firemaking", 1]
            if (level < Tables.int("logs.${item.id}.level")) {
                message("You need a Firemaking level of $level to burn this.")
                return@itemOnObject
            }
            player.addXp(Skill.Firemaking, Tables.int("logs.${item.id}.xp"))
        }
    }
}
```
