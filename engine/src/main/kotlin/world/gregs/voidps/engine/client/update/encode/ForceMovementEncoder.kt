...
class ForceMovementEncoder(private val npc: Boolean, mask: Int) : VisualEncoder<ForceMovement>(mask) {
    ...
    override fun encode(writer: Writer, visual: ForceMovement) {
        ...
        writer.apply {
            ...
            writeByte(tile2.x, if (npc) Modifier.INVERSE else Modifier.INVERSE)
            writeByte(tile2.y, if (npc) Modifier.INVERSE else Modifier.INVERSE)
            writeShort(delay1, Modifier.ADD, order = if (npc) Endian.BIG else Endian.LITTLE)
