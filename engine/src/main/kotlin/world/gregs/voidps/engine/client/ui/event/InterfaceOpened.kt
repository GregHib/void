...
fun interfaceOpen(vararg ids: String = arrayOf("*"), override: Boolean = true, block: suspend InterfaceOpened.(Player) -> Unit) {
    for (id in ids) {
        Events.handle("interface_open", id, override = override, handler = block)
    }
}