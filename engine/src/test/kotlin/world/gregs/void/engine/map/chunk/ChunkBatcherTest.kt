...
import world.gregs.void.network.codec.game.encode.ChunkClearEncoder
import world.gregs.void.network.codec.game.encode.ChunkUpdateEncoder
...
internal class ChunkBatcherTest : KoinMock() {
    ...
    fun `Send chunk clear message`() {
        // Given
        mockkStatic("world.gregs.void.engine.client.SessionsKt")
        val player: Player = mockk(relaxed = true)
        val chunk = Chunk(11, 11, 1)
        every { batcher.getChunkOffset(player, chunk) } returns Chunk(7, 7)
        // When
