...
class PostgresStorageTest : DatabaseTest {

    private val storage = PostgresStorage()

    private val save = PlayerSave(
        name = "durial_321",
    ...
    fun `Check if account name exists`() {
        transaction {
            for (account in AccountsTable.selectAll().where { AccountsTable.name eq save.name }.toList()) {
                println("${account[AccountsTable.name]} - ${account[AccountsTable.id]}")
            }
        }
