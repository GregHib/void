...
internal class InterfaceParentTest : InterfaceTest() {
    ...
    fun `Resizable has different parent`() {
        ...
        interfaces[4] = InterfaceDetail(id = 4, data = InterfaceData(resizableParent = 10, resizableIndex = ROOT_INDEX))
    ...
    fun `Interface without parent throws error`(resizable: Boolean) {
        interfaces[7] = InterfaceDetail(id = 7, data = InterfaceData(fixedIndex = ROOT_INDEX, resizableIndex = ROOT_INDEX))
    ...
    fun `Can't open child if parent not open`() {
        ...
        interfaces[1] = InterfaceDetail(id = 1, data = InterfaceData(fixedParent = 0, fixedIndex = ROOT_INDEX))
