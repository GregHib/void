...
value class ZoneObject(val packed: Int) {
    ...
    override fun toString(): String {
        return "ZoneObject(id=$id, x=$x, y=$y, level=$level, shape=$shape, rotation=$rotation)"
    }

    companion object {

        fun pack(id: Int, x: Int, y: Int, level: Int, shape: Int, rotation: Int): Int {
            return x  + (y shl 3) + (level shl 6) + (rotation shl 8) + (shape shl 10) + (id shl 15)
        }
        ...
        fun level(packed: Int): Int = packed shr 6 and 0x3
