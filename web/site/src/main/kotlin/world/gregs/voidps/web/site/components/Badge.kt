...
fun Ui.badge(
    ...
    receiver.span {
        ...
        if (showWhen != null) {
            // x-show clears any inline `display` it doesn't own (see base.css).
            attributes["class"] = "void-inline-flex"
        }
        val display = if (showWhen != null) "" else "display:inline-flex;"
        style = "${display}align-items:center;gap:var(--space-3);padding:0 10px;height:20px;" +
