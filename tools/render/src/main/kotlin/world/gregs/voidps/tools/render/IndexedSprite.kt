...
internal class IndexedSprite {
    ...
    companion object {
        fun method1512(cache: Cache, i: Int): IndexedSprite? {
            val `is` = cache.data(Index.SPRITES, i)
            if (`is` == null) return null
            return method1517(`is`)[0]
        }

        private fun method1517(`is`: ByteArray): Array<IndexedSprite> {
            val packet = Packet(`is`)
            packet.pos = `is`.size - 2
            val i = packet.readUnsignedShort()
            val indexedSprites: Array<IndexedSprite> = Array(i) { IndexedSprite() }
            ...
            for (i_76_ in 0..<i) indexedSprites[i_76_].anInt2703 = packet.readUnsignedShort()
            for (i_77_ in 0..<i) indexedSprites[i_77_].anInt2700 = packet.readUnsignedShort()
            for (i_78_ in 0..<i) indexedSprites[i_78_].anInt2702 = packet.readUnsignedShort()
            for (i_79_ in 0..<i) indexedSprites[i_79_].anInt2696 = packet.readUnsignedShort()
            for (i_80_ in 0..<i) {
                val class207 = indexedSprites[i_80_]
            ...
            for (i_83_ in 0..<i) indexedSprites[i_83_].anIntArray2697 = is_81_
            packet.pos = 0
            for (i_84_ in 0..<i) {
                val class207 = indexedSprites[i_84_]
