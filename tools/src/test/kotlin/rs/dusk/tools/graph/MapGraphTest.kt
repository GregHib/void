...
internal class MapGraphTest {
    ...
    fun setup() {
        ...
        graph = MapGraph(mockk(), mockk(), mockk(), mockk(), mockk())
    }
    ...
    fun `Flood fill 2x2 ignore collision`() {
        every { strategy.blocked(Tile(0, 0), Direction.NORTH_EAST) } returns true
    ...
    fun `Find two points for two separated knots of three tiles each`() {
        for(x in 0 until 3) {
            every { strategy.blocked(Tile(x, 1), Direction.NONE) } returns true
            every { strategy.blocked(Tile(x, 0), Direction.NORTH) } returns true
            every { strategy.blocked(Tile(x, 0), Direction.NORTH_EAST) } returns true
            every { strategy.blocked(Tile(x, 0), Direction.NORTH_WEST) } returns true
            every { strategy.blocked(Tile(x, 2), Direction.SOUTH) } returns true
    ...
    fun `Find closest free tile to center point`() {
        val center = Tile(1, 1)
        every { strategy.blocked(center, Direction.NONE) } returns true
