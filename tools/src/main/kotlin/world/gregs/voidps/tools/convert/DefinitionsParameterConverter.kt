...
object DefinitionsParameterConverter {
    ...
    fun convert(target: File, other: File) {
        ...
        val itemDefinitions718 = ItemDecoder718().loadCache(otherCache)
        val npcDefinitions718 = NPCDecoder718().loadCache(otherCache)
        val objectDefinitions718 = ObjectDecoder718().loadCache(otherCache)

        val itemDecoder = ItemDecoderFull()
        val itemDefinitions = itemDecoder.loadCache(cache)
        ...
        val npcDefinitions = npcDecoder.loadCache(cache)
        val npcDecoderMembers = NPCDecoderFull(members = true)
        val npcDefinitionsMembers = npcDecoderMembers.loadCache(cache)
        ...
        val objectDefinitions = objectDecoder.loadCache(cache)
        val objectDecoderMembers = ObjectDecoderFull(members = true)
        val objectDefinitionsMembers = objectDecoderMembers.loadCache(cache)
