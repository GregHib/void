...
class WorldMapLinker(
    ...
    override fun process(content: Map<Tile, List<GameObject>>) {
        ...
        addMapLinks(content)
        println("${graph.links.size - start} world map links found.")
    }
        ...
        for (i in ints.indices) {
            ...
            if (int == BI_DIRECTIONAL_LINK) {
                ...
                if (ones.isNotEmpty() && twos.isNotEmpty()) {
                    ...
                    val end = twos.first()
                    val s = linker.getAvailableTiles(start).firstOrNull()
                    val e = linker.getAvailableTiles(end).firstOrNull()
                    if (s != null && e != null) {
                        var option = getFirstOption(start.id)
                        if (!graph.tracked(start, option)) {
                            val link = graph.addLink(s, e)
                            link.actions = mutableListOf("object ${start.id} $option")
                        }
                        option = getFirstOption(end.id)
                        if (!graph.tracked(end, option)) {
                            val link = graph.addLink(e, s)
                            link.actions = mutableListOf("object ${end.id} $option")
                        }
                    } else {
    ...
    private fun addUniLink(content: Map<Tile, List<GameObject>>, tile: Tile, tile2: Tile, radius: Int) {
        ...
        if (ones.isNotEmpty()) {
            val start = ones.first()
            val t = linker.getAvailableTiles(start).firstOrNull()
            if (t != null) {
                val option = getFirstOption(start.id)
                if (!graph.tracked(start, option)) {
                    val link = graph.addLink(t, tile2)
                    link.actions = mutableListOf("object ${start.id} $option")
                }
            } else {
                println("Unknown $tile |-> $tile2")
