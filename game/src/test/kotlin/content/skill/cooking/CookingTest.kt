...
internal class CookingTest : WorldTest() {
    ...
    @Test
    fun `Cooking has a chance at burning`() {
        ...
        player.inventory.add("raw_shrimps", 2)
        val fire = createObject("fire_orange", emptyTile.addY(1))
        ...
        assertEquals(1, player.inventory.count("raw_shrimps"))
        assertEquals(0, player.inventory.count("shrimps"))
        assertEquals(1, player.inventory.count("burnt_shrimp"))
        assertEquals(0.0, player.experience.get(Skill.Cooking))
    }
