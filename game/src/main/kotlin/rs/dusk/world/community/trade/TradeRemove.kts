...
InterfaceOption where { name == "trade_main" && component == "offer_options" && option == "Remove" } then {
    remove(player, itemId, itemIndex, 1)
}

InterfaceOption where { name == "trade_main" && component == "offer_options" && option == "Remove-5" } then {
    remove(player, itemId, itemIndex, 5)
}

InterfaceOption where { name == "trade_main" && component == "offer_options" && option == "Remove-10" } then {
    remove(player, itemId, itemIndex, 10)
}
...
InterfaceOption where { name == "trade_main" && component == "offer_options" && option == "Remove-All" } then {
    val amount = player.offer.getAmount(itemIndex)// TODO count not amount
    remove(player, itemId, itemIndex, amount)
}
