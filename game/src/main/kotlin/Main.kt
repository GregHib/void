...
object Main {
    ...
    private fun cache(cache: Cache, files: ConfigFiles): Module {
        ...
        val module = module {
            ...
            single(createdAtStart = true) { ObjectDefinitions.init(ObjectDecoder(members, lowDetail = false, get<ParameterModifier>()).load(cache)).load(files.list(Settings["definitions.objects"])) }
            single(createdAtStart = true) { NPCDefinitions.init(NPCDecoder(members, get<ParameterModifier>()).load(cache)).load(files.list(Settings["definitions.npcs"]), get()) }
            single(createdAtStart = true) { ItemDefinitions.init(ItemDecoder(get<ParameterModifier>()).load(cache)).load(files.list(Settings["definitions.items"])) }
            ...
            single(createdAtStart = true) { StructDefinitions.init(StructDecoder(get<ParameterModifier>()).load(cache)).load(files.find(Settings["definitions.structs"])) }
            ...
            single(createdAtStart = true) { ParameterModifier(get(), get()) }
