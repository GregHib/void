...
on<Registered>({ isGodsword(it.equipped(EquipSlot.Weapon)) }) { player: Player ->
    updateWeapon(player, player.equipped(EquipSlot.Weapon))
}

on<ItemChanged>({ container == "worn_equipment" && index == EquipSlot.Weapon.index && isGodsword(item) }) { player: Player ->
...
on<CombatHit>({ isGodsword(weapon) }) { player: Player ->
