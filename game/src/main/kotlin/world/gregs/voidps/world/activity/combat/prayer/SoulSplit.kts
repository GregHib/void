...
on<CombatDamage>({ it.hasEffect("prayer_soul_split") && damage > 0 }) { player: Player ->
    ...
    player.shoot("soul_split", target, height = 10, endHeight = 10, delay = 0, flightTime = if (closeCombat) 40 else 80, curve = 15)
    delay(target, if (closeCombat) 1 else 2) {
        target.setGraphic("soul_split_hit")
        target.shoot("soul_split", player, height = 10, endHeight = 10, delay = 0, flightTime = if (closeCombat) 40 else 80, curve = 15)
    }
}