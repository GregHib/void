...
on<ItemChanged>({ container == "worn_equipment" && index == EquipSlot.Weapon.index && isStaff(item) }) { player: Player ->
...
on<CombatSwing>({ !swung() && isStaff(it.weapon) }, Priority.LOW) { player: Player ->
    player.setAnimation("staff_attack")
    player.hit(target)
    delay = 4
}

on<CombatHit>({ isStaff(weapon) }, Priority.LOW) { player: Player ->
