...
on<world.gregs.voidps.world.interact.entity.player.combat.consume.Consume>({ item.id == "bandits_brew" }) { player: Player ->
...
on<world.gregs.voidps.world.interact.entity.player.combat.consume.Consume>({ item.id == "beer" }) { player: Player ->
    player.levels.boost(Skill.Strength, 1, 0.02)
    player.levels.drain(Skill.Attack, 1, 0.06)
}

on<world.gregs.voidps.world.interact.entity.player.combat.consume.Consume>({ item.id.startsWith("keg_of_beer") }) { player: Player ->
...
on<world.gregs.voidps.world.interact.entity.player.combat.consume.Consume>({ item.id == "grog" }) { player: Player ->
