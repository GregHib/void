...
consume({ item.id == "bandits_brew" }) { player: Player ->
...
consume({ item.id == "beer" }) { player: Player ->
    player.levels.boost(Skill.Strength, 1, 0.02)
    player.levels.drain(Skill.Attack, 1, 0.06)
}

consume({ item.id.startsWith("keg_of_beer") }) { player: Player ->
...
consume({ item.id == "grog" }) { player: Player ->
