...
fun isSpear(item: Item?) = item != null && (item.name.endsWith("spear") || item.name.endsWith("hasta") || item.name.endsWith("halberd"))
...
on<CombatSwing>({ !swung() && isSpear(it.weapon) }, Priority.LOWER) { player: Player ->
    ...
    player.hit(target)
    delay = if (player.weapon.name.endsWith("halberd")) 7 else 4
}
