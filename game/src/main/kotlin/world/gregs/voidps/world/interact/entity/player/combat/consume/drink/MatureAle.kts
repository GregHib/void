...
consume("asgarnian_ale*") { player: Player ->
...
consume("axemans_folly*") { player: Player ->
    val mature = item.id.endsWith("_m")
...
consume("chefs_delight*") { player: Player ->
    val mature = item.id.endsWith("_m")
...
consume("dragon_bitter*") { player: Player ->
    val mature = item.id.endsWith("_m")
    player.levels.boost(Skill.Strength, if (mature) 3 else 2)
    player.levels.drain(Skill.Attack, if (mature) 6 else 4)
}

consume("dwarven_stout*") { player: Player ->
    val mature = item.id.endsWith("_m")
...
consume("greenmans_ale*") { player: Player ->
    val mature = item.id.endsWith("_m")
...
consume("slayers_respite*") { player: Player ->
    val mature = item.id.endsWith("_m")
...
consume("*wizards_mind_bomb") { player: Player ->
    val mature = item.id.startsWith("mature_")
