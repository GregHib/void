...
on<ObjectOption>({ operate && target.id == "varrock_manhole" && option == "Open" }) { player: Player ->
    arriveDelay()
...
on<ObjectOption>({ operate && target.id == "varrock_manhole_open" && option == "Close" }) { player: Player ->
    arriveDelay()
