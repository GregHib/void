...
suspend fun Interaction.madeSword() {
    ...
    npc<OldCheerful>("You're welcome - thanks for the pie!")
}

suspend fun Interaction.replacementSword() {
    player<Cheerful>("Can you make that replacement sword now?")
    npc<OldUnsure>("How are you doing finding those sword materials?")
    ...
    if (player.inventory.contains("blurite_ore")) {
        ...
        npc<OldCheerful>("Better go get some then, huh?")
        return
    }
...
suspend fun Interaction.aboutSword() {
    ...
    npc<OldUnsure>("Have you got a picture of the sword for me yet?")
    ...
    npc<OldTalking>("Well, come back when you do.")
}
