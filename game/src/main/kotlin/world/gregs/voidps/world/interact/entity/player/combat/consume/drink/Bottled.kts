...
consume({ item.id == "karamjan_rum" }) { player: Player ->
    player.levels.boost(Skill.Strength, 5)
    player.levels.drain(Skill.Attack, 4)
}

consume({ item.id == "vodka" || item.id == "gin" || item.id == "brandy" || item.id == "whisky" }) { player: Player ->
    player.levels.boost(Skill.Strength, 1, 0.05)
    player.levels.drain(Skill.Attack, 3, 0.02)
}

consume({ item.id == "bottle_of_wine" }) { player: Player ->
    player.levels.drain(Skill.Attack, 3)
}

consume({ item.id == "braindeath_rum" }) { player: Player ->
