...
on<Consume>({ item.id.startsWith("asgarnian_ale") }) { player: Player ->
...
on<Consume>({ item.id.startsWith("axemans_folly") }) { player: Player ->
    val mature = item.id.endsWith("_m")
...
on<Consume>({ item.id.startsWith("chefs_delight") }) { player: Player ->
    val mature = item.id.endsWith("_m")
...
on<Consume>({ item.id.endsWith("cider") }) { player: Player ->
...
on<Consume>({ item.id.startsWith("dragon_bitter") }) { player: Player ->
    val mature = item.id.endsWith("_m")
    player.levels.boost(Skill.Strength, if (mature) 3 else 2)
    player.levels.drain(Skill.Attack, if (mature) 6 else 4)
}

on<Consume>({ item.id.startsWith("dwarven_stout") }) { player: Player ->
    val mature = item.id.endsWith("_m")
...
on<Consume>({ item.id.startsWith("greenmans_ale") }) { player: Player ->
    val mature = item.id.endsWith("_m")
...
on<Consume>({ item.id.startsWith("slayers_respite") }) { player: Player ->
    val mature = item.id.endsWith("_m")
...
on<Consume>({ item.id.endsWith("wizards_mind_bomb") }) { player: Player ->
    val mature = item.id.startsWith("mature_")
