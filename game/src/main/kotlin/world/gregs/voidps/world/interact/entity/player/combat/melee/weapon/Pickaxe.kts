...
on<Registered>({ isPickaxe(it.equipped(EquipSlot.Weapon)) }) { player: Player ->
    updateWeapon(player, player.equipped(EquipSlot.Weapon))
}

on<ItemChanged>({ container == "worn_equipment" && index == EquipSlot.Weapon.index && isPickaxe(item) }) { player: Player ->
...
on<CombatHit>({ isPickaxe(weapon) }, Priority.LOW) { player: Player ->
