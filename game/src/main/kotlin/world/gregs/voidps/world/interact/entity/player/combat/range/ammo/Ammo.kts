...
combatSwing({ player -> player.fightStyle == "range" }, Priority.HIGHEST) { player: Player ->
...
combatSwing({ player -> player.fightStyle == "range" && Weapon.isBowOrCrossbow(player.weapon) && Ammo.required(player.weapon) }, Priority.HIGHEST) { player: Player ->
    player["required_ammo"] = player.weapon.def["ammo_required", 1]
}

combatSwing({ player -> player.fightStyle == "range" && Weapon.isBowOrCrossbow(player.weapon) && Ammo.required(player.weapon) }, Priority.HIGH) { player: Player ->
...
combatSwing({ player -> player.fightStyle == "range" && !Ammo.required(player.weapon) }, Priority.HIGH) { player: Player ->
    player.ammo = when {
