...
on<ItemChanged>({ container == "worn_equipment" && isSetSlot(index) && it.hasVar("void_set_effect") && !isVoid(item) }) { player: Player ->
    player.clearVar("void_set_effect")
}

on<ItemChanged>({ container == "worn_equipment" && isSetSlot(index) && !it.hasVar("void_set_effect") && isVoid(item) && it.hasFullSet("") }) { player: Player ->
    player.setVar("void_set_effect", true)
}

on<ItemChanged>({ container == "worn_equipment" && isSetSlot(index) && it.hasVar("elite_void_set_effect") && !isEliteVoid(item) }) { player: Player ->
    player.clearVar("elite_void_set_effect")
}

on<ItemChanged>({ container == "worn_equipment" && isSetSlot(index) && !it.hasVar("elite_void_set_effect") && isEliteVoid(item) && it.hasFullSet("elite_") }) { player: Player ->
...
on<HitEffectiveLevelModifier>({ it.hasVar("void_set_effect") || it.hasVar("elite_void_set_effect") }, priority = Priority.LOW) { player: Player ->
