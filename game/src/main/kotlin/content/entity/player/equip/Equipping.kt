...
class Equipping : Script {
    ...
    companion object {
        ...
        private fun replaceWeaponShieldWith2h(player: Player, item: ItemDefinition) = player.has(EquipSlot.Shield) && player.has(EquipSlot.Weapon) && item.type == EquipType.TwoHanded

        private fun replaceShieldWith2h(player: Player, item: ItemDefinition) = player.has(EquipSlot.Shield) && !player.has(EquipSlot.Weapon) && item.type == EquipType.TwoHanded

        private fun replace2hWithShield(player: Player, item: ItemDefinition) = player.equipped(EquipSlot.Weapon).type == EquipType.TwoHanded && item.slot == EquipSlot.Shield

        private fun getOtherHandSlot(slot: EquipSlot) = if (slot == EquipSlot.Shield) EquipSlot.Weapon else EquipSlot.Shield

        private fun updateWeaponEmote(player: Player) {
        ...
        private fun playEquipSound(player: Player, item: ItemDefinition) {
        ...
        fun equip(player: Player, item: Item, slot: Int): Boolean {
            val def = item.def
            ...
            when (player.inventory.transaction.error) {
                TransactionError.None -> {
                    ...
                    logger.warn { "Failed to equip item $player $item ${item.slot.index}" }
                    return false
                }
            }
        }
