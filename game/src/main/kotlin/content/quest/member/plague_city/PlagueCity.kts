...
questJournalOpen("plague_city") {
    val lines = when (player.quest("plague_city")) {
        "started" -> listOf(
            ...
            //todo if you dont have the dwellberries
            "<navy>I have the <maroon>Dwellberries <navy>she needs to make it.",
            "<navy>I should give them to her.",
            "",
            "",
        )
        "has_mask" -> listOf(
        ...
        "grill_open" -> {
            ...
            if (player["plaguecity_picture_asked", false]) {
                ...
                if (player.holdsItem("picture_plague_city")) {
                    ...
                    list.add("")//todo also check if in bank
                }
            } else {
        ...
        "spoken_to_jethick" -> {
            ...
            if (!player.holdsItem("book_turnip_growing_for_beginners")) {//todo check if its in bank
        ...
        "completed", "completed_with_spell"  -> listOf(//todo with completed check if you don't have a_magic_scroll if it says you can get another from Edmond
            "<str>I've spoken to Edmond in East Ardougne. He's asked to",
            "<str>help find his daughter Elena, who's gone missing while ",
            "<str>trying to help plague victims in West Ardougne.",
            "<str>Alrena has given me a Gas Mask to protect me from the",
...
suspend fun SuspendableContext<Player>.ropeOnGrill() {
    if (player.quest("plaguecity_pipe") == "grill_rope") {
        player.message("todo check what happens if you use rope on grill again")//todo
    } else {
...
inventoryItem("Read", "a_magic_scroll") {//todo what if i had one dropped then read it after
