...
class OldCrone : Script {
    ...
    init {
        ...
        itemOnNPCOperate("cup_of_milky_tea_ghosts_ahoy,cup_of_tea_ghosts_ahoy,cup_of_milky_tea,cup_of_tea_nettle_tea,nettle_tea,milky_nettle_tea,translation_manual,mystical_robes,book_of_haricanto", "ahoy_crone") { interaction ->
            when {
                ...
                ENCHANTMENT_PRIORITY.contains(interaction.item.id) -> stageFourOptions(interaction.target)
            }
        }
    }
    ...
    private suspend fun Player.offerTea(itemUsed: Item) {
        ...
        npc<Neutral>("Yes, Velorina was once a very good friend! It has been many years since we spoke last. How is she?")
        player<Neutral>("She's a ghost, I'm afraid.")
        npc<Neutral>("They are all dead, then. Even Velorina. I should have done something to stop what was happening, instead of running away.")
        player<Neutral>("She and many others want to pass on into the next world, but Necrovarus will not let them. Do you know of any way to help them?")
    ...
    private suspend fun Player.explainEnchantmentNeeds() {
        npc<Neutral>("There is an enchantment that I can perform on such an amulet that will give it the power of command over ghosts. It will work only once, but it will enable you to command Necrovarus to let the ghosts pass on.")
    ...
    private suspend fun Player.stageFourOptions(target: NPC) {
        val hasManual = inventory.contains("translation_manual") && !get("ahoy_given_manual", false)
        val hasRobes = inventory.contains("mystical_robes") && !get("ahoy_given_robes", false)
        val hasBook = inventory.contains("book_of_haricanto") && !get("ahoy_given_book", false)
        if (hasManual || hasRobes || hasBook) {
            player<Neutral>("I have something for you.")
        }
        if (hasManual) {
            acceptQuestItem("translation_manual", "ahoy_given_manual", "A translation manual - yes, this should do the job.")
        }
        if (hasRobes) {
            acceptQuestItem("mystical_robes", "ahoy_given_robes", "Good - the robes of Necrovarus.")
        }
        if (hasBook) {
            acceptQuestItem("book_of_haricanto", "ahoy_given_book", "The Book of Haricanto! I have no idea how you came by this, but well done!!")
        }
            ...
            if (toyStage >= 2) {
                ...
                if (inventory.contains("model_ship") || inventory.contains("model_ship_silk")) {
                    option<Sad>("I am afraid I have not found your son yet.") {
                        npc<Neutral>("I never expected that you would find him - although if you do, please let me know.")
                    }
                } else {
    ...
    private suspend fun Player.sonStory() {
        npc<Neutral>("I have lived here on my own for many years, but not always. When I left Port Phasmatys, I took my son with me. He grew up to be a fine boy, always in love with the sea.")
        npc<Neutral>("He was about twelve years old when he ran away with some pirates to be a cabin boy. I never saw him again.")
        ...
        set("ahoy_mast_top", random.nextInt(1..6))
        set("ahoy_mast_bottom", random.nextInt(1..6))
        set("ahoy_mast_skull", random.nextInt(1..6))
        addOrDrop("model_ship")
        item(item = "model_ship", text = "The old woman gives you a toy boat.")
        player<Neutral>("Was this his boat?")
        npc<Neutral>("Yes, he made it himself. It is a model of the very ship in which he sailed away. The paint has peeled off and it has lost its flag, but I could never throw it away.")
        player<Neutral>("If I find him, I will pass it on.")
    }

    private suspend fun Player.foundSon() {
        npc<Neutral>("Goodness! Where is he?")
        player<Neutral>("He lives on a shipwreck to the east of here. He remembers you and wishes you well.")
        npc<Neutral>("Oh thank you!! I will travel to see him as soon as I am able!!")
    }

    private suspend fun Player.performEnchantment(target: NPC) {
        if (!carriesItem("ghostspeak_amulet")) {
            return npc<Neutral>("You don't have a ghostspeak amulet for me to enchant.")
        }
        target.anim("human_dancing")
        delay(3)
        if (equipment.contains("ghostspeak_amulet")) {
            equipment.replace(EquipSlot.Amulet.index, "ghostspeak_amulet", "ghostspeak_amulet_enchanted")
    ...
    private suspend fun Player.enchantmentFollowUp(target: NPC) {
    ...
    private suspend fun Player.postQuestThanks() {
        ...
        npc<Neutral>("So Necrovarus has let the ghosts go free.... You have achieved many wonderful things, my young friend.")
    }
