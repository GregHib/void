 ...
 * Returns false when [potion] is not a known potion, which is what marks an item as one, so a
 * potion with no effect yet still has a branch.
 */
fun Player.potionEffects(potion: String): Boolean {
    when (potion.substringBeforeLast('_')) {
        ...
        "super_attack", "super_attack_mix", "super_attack_potion_stealing_creation", "cw_super_attack_potion" -> levels.boost(Skill.Attack, 5, 0.15)
        "super_strength", "super_strength_mix", "super_strength_potion_stealing_creation", "cw_super_strength_potion" -> levels.boost(Skill.Strength, 5, 0.15)
        "super_defence", "super_defence_mix", "cw_super_defence_potion" -> levels.boost(Skill.Defence, 5, 0.15)
        "super_magic_potion", "super_magic_mix", "magic_potion_stealing_creation", "cw_super_magic_potion" -> levels.boost(Skill.Magic, 5, 0.15)
        "super_ranging_potion", "super_ranging_mix", "ranging_potion_stealing_creation", "cw_super_ranging_potion" -> levels.boost(Skill.Ranged, 4, 0.10)
        ...
        "prayer_potion", "prayer_mix", "prayer_potion_stealing_creation" -> levels.restore(Skill.Prayer, 7, if (hasHolyItem()) 0.27 else 0.25)
        ...
        // TODO juju cooking (no burn chance) and juju hunter (herblore habitat catch bonus) effects.
        "juju_cooking_potion", "juju_hunter_potion" -> {}
        // Used on something else rather than drunk, so drinking them does nothing.
        "compost_potion", "guthix_balance", "serum_207", "serum_208", "olive_oil", "sacred_oil" -> {}
        else -> return false
    }
    return true
}
