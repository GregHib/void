...
class ZavisticRarve : Script {
    ...
    init {
        ...
        npcOperate("Talk-to", "zavistic_rarve") { (target) ->
            ...
            if (sandProgress < 40 && zogreProgress < 4) {
                ...
                npc<Neutral>("Well... I am, I suppose, anyway... we're very busy here, hurry up, what do you want?")
            } else {
                npc<Neutral>("What are you doing...Oh, it's you...sorry...didn't realise...what can I do for you?")
            }
            mainMenu()
        }
        ...
        objectOperate("Ring", "zogre_outdoor_bell") { (target) ->
            ...
            if (sandProgress < 40 && zogreProgress < 4) {
                ...
                npc<Neutral>("Well...anyway...we're very busy here, hurry up what do you want?")
            } else {
                npc<Neutral>("What are you doing...Oh, it's you...sorry...didn't realise...what can I do for you?")
            }
            mainMenu()
        ...
        itemOnNPCOperate("black_prism", "zavistic_rarve") {
            player<Neutral>("I found this black prism at Jiggig where the undead ogre activity was happening?")
            showBlackPrism()
        }

        itemOnNPCOperate("torn_page", "zavistic_rarve") {
            player<Neutral>("I think I've found a clue from the Jiggig area where the undead ogre activity is happening.")
            showTornPage()
        }
        ...
        itemOnNPCOperate("zogre_sithik_portrait_bad", "zavistic_rarve") {
            ...
            npc<Neutral>("Who the demonikin is that? Is it meant to be a portrait of Sithik, it doesn't look anything like him!")
        }
    }
    ...
    fun ChoiceOption.aboutSand(): Unit = option<Neutral>("I have a rather sandy problem that I'd like to palm off on you.") {
        sendSandChat()
    }
    ...
    fun ChoiceOption.whatIsThereToDo(): Unit = option<Quiz>("What is there to do in the Wizards' Guild?") {
        npc<Neutral>("This is the finest wizards' establishment in the land. We have magic portals to the other towers of wizardry around RuneScape. We have a particularly wide collection of runes in our rune shop. We sell some of")
        npc<Neutral>("the finest mage robes in the land and we have a training area full of zombies for you to practice your magic on.")
        guildMenu()
    }

    fun ChoiceOption.whatAreRequirements(): Unit = option<Quiz>("What are the requirements to get in the Wizards' Guild?") {
        npc<Neutral>("You need a magic level of 66, the high magic energy level is too dangerous for anyone below that level.")
        guildMenu()
    }

    fun ChoiceOption.whatDoYouDo(): Unit = option<Neutral>("What do you do in the Guild?") {
        npc<Neutral>("I'm the Grand Secretary for the Wizards' Guild, I have lots of correspondence to keep up with, as well as attending to the discipline of the more problematic guild members.")
        guildMenu()
    }
        ...
        when (get("hand_in_the_sand", 0)) {
            ...
            120 -> {
                if (inventory.contains("magical_orb_active")) {
                    ...
                    npc<Neutral>("It's ok, I saw the whole thing as the orb is connected via magic to me as I enchanted it.")
                    npc<Angry>("I think this Sandy needs a lesson, please bring me 5 earth runes and a bucket of sand.")
                    runesAndSandRequest()
                }
            }
            130 -> {
...
//                    sendHandQuestReward()
    ...
    private suspend fun Player.handInTheSandHandReveal() {
        ...
        player<Neutral>("The Guard Captain asked me to see if you have any... missing... wizards.")
    ...
    private suspend fun Player.magicScrollReveal() {
        ...
        player<Neutral>("I talked to Bert and found something very strange about his hours.")
        npc<Quiz>("Oh? Did he kill Clarence?")
        player<Neutral>("No, but he doesn't remember changing his hours, and his rota and the original that his boss Sandy had, are different! ")
        player<Neutral>("... oh, and this scroll appeared when they changed - he gave it to me.")
        npc<Happy>("I recognise that type of scroll! It's used in a mind altering spell of some sort. Did you speak to this... Sandy guy? Perhaps he has a hand in this.")
    ...
    private suspend fun Player.helpMoreFlow() {
        if (get("handsand_tele", false)) {
            npc<Sad>("Unfortunately I've already helped you with one teleport, get some exercise - your legs won't fall off!")
            return
        }
        ...
        npc<Quiz>("Ok, would you like me to transport you to Port Sarim? I'm sticking my neck out a bit helping you like this and can only do it once though!")
        choice {
            yesTeleport()
            noTeleport()
        }
    ...
    private suspend fun Player.replaceOrb() {
        ...
        if (get("hand_in_the_sand", 0) == 110) {
            ...
            addOrDrop("magical_orb")
            npc<Neutral>("No matter, here, have another and please hurry, whoever killed Clarence must pay!")
        }
    }
    ...
    private suspend fun Player.sendZogreChat() {
        ...
        when {
            ...
            inventory.contains("black_prism") && inventory.contains("torn_page") -> {
                player<Neutral>("There's some undead ogre activity over at Jiggig, I've found some clues, I wondered if you'd have a look at them.")
                showBothClues()
            }
            inventory.contains("black_prism") -> {
                player<Neutral>("There's some undead ogre activity over at 'Jiggig', and the ogres have asked me to look into it. I think I've found a clue and I wonder if you could take a look at it for me?")
                showBlackPrism()
            }
            inventory.contains("torn_page") -> {
                player<Neutral>("There's some undead ogre activity over at Jiggig, I've found a clue that you may be able to help with.")
                showTornPage()
            }
            else -> guildMenu()
        }
    ...
    fun ChoiceOption.whatDidYouSayShouldDo(): Unit = option<Neutral>("What did you say I should do?") {
        npc<Neutral>("You should go and have a chat with Sithik Ints, he's in that house just to the north. He's a lodger and has a room upstairs. Just tell him that I sent you to see him. He should be fine once you've mentioned my name.")
        sithikInvestigationMenu()
    }

    fun ChoiceOption.whereIsSithik(): Unit = option<Neutral>("Where is Sithik?") {
        npc<Neutral>("He's in that house just to the north, less than a few seconds walk away. He's a lodger and has a room upstairs...he's not very well though.")
        sithikInvestigationMenu()
    }
    ...
    fun ChoiceOption.canYouHelp(): Unit = option<Neutral>("Can you help me?") {
        npc<Neutral>("I'm happy to help as much as I can but you have to remember that I'm quite busy. If you find any more clues about what happened at Jiggig, I'll consider them with an open mind - that's as much as I can offer.")
        sithikInvestigationMenu()
    }

    fun ChoiceOption.wantToAskAboutGuild(): Unit = option<Neutral>("I want to ask about the Magic Guild.") {
    ...
    private suspend fun Player.showBlackPrism() {
        ...
        if (get("thzfe_prismsearch", 0) >= 4) {
            npc<Neutral>("Yes, you've already showed me that, bring it to me when you've resolved the problems at Jiggig and I'll see what I can do.")
            return
        }
        npc<Neutral>("Hmmm, well this is an uncommon spell component. On it's own it's useless, but with certain necromantic spells it can be very powerful. Did you find anything else there?")
        if (inventory.contains("dragon_inn_tankard")) {
            item(item = "dragon_inn_tankard", text = "You show the tankard to Zavistic.")
            player<Neutral>("Well, I found this...")
            npc<Neutral>("Hmmm, no, that's not really associated with this to be honest. Did you find anything else there?")
            player<Neutral>("Not really.")
        } else {
            player<Neutral>("Not really.")
        }
        npc<Neutral>("I don't know what to say then, there isn't enough to go on with the clues you've shown me so far. I'd suggest going back to search a bit more, but you may just be wasting your time?")
        npc<Neutral>("Hmm, but this prism does seem to have some magical protection. Once you've finished with this item, bring it back to me would you? I may have a reward for you!")
    ...
    private suspend fun Player.showTornPage() {
        ...
        npc<Neutral>("Hmm, this is a half torn spell page, it requires another spell component to be effective. Did you find anything else there?")
    ...
    private suspend fun Player.showBothClues() {
        ...
        player<Neutral>("I got them from a nearby Ogre tomb, it's recently been infested with zombie ogres and I'm trying to work out what happened there.")
        npc<Sad>("This is very troubling $name, very troubling indeed. While it's permitted for learned members of our order to research the 'dark arts', it's absolutely forbidden to make use of such magic.")
        player<Neutral>("Do you have any leads on people that I might talk to regarding this?")
        set("thzfe_prismsearch", 4)
        npc<Neutral>("Well a wizard by the name of 'Sithik Ints' was doing some research in this area. He may know something about it. He's lodged at that guest house to the North, though he's ill and isn't able to leave his room.")
        npc<Neutral>("Why not go and talk to him, poke around a bit and see if anything comes up. Let me know how you get on. However, I doubt that 'Sithik' had anything to do with it.")
        npc<Neutral>("There's a severe penalty for using the 'dark arts'. If you find any evidence to the contrary, please bring it to me.")
        npc<Neutral>("Hmm, that black prism seems to have some magical protection. Once you've finished with this item, bring it back to me would you. I may have a reward for you.")
    }
    ...
    private suspend fun Player.showTankard() {
        ...
        if (get("thzfe_innkeepermugshown", false)) {
            npc<Neutral>("Yeah, you've shown me this before...if this is all the evidence you have?")
        ...
        npc<Neutral>("That doesn't mean anything in itself, you could have gotten that from anywhere. Even from the Dragon Inn tavern! There isn't anything to link Brentle Vahn with Sithik Ints.")
    }

    private suspend fun Player.showNecromancyBook() {
        player<Neutral>("I have this necromancy book as evidence that Sithik is involved with the undead ogres at Jiggig.")
        if (get("thzfe_shownnecrobook", false)) {
            npc<Neutral>("Yeah, you've shown me this before...if this is all the evidence you have?")
            player<Neutral>("Please just look at it again...")
            npc<Neutral>("Ok, let me look then.")
        }
        npc<Neutral>("Ok, so he's researching necromancy...it doesn't mean anything in itself.")
        player<Neutral>("Yes, but if you look, you can see that there is a half torn page which matches the page I found at Jiggig.")
        set("thzfe_shownnecrobook", true)
        npc<Neutral>("Hmm, yes, but someone could have stolen that from him and then gone and cast it without his permission or to try and deliberately implicate him.")
    }

    private suspend fun Player.showHamBook() {
        player<Neutral>("Look, this book proves that Sithik hates all monsters and most likely Ogres with a passion.")
        ...
        npc<Neutral>("So what, hating monsters isn't a crime in itself...although I suppose that it does give a motive if Sithik was involved. On its own, it's not enough evidence though.")
    }

    private suspend fun Player.showSignedPortrait() {
        player<Neutral>("This is a portrait of Sithik, signed by the landlord of the Dragon Inn saying that he saw Sithik and Brentle Vahn together.")
    ...
    private suspend fun Player.notYetUsedPotion() {
        ...
        npc<Neutral>("Try to use the potion on Sithik somehow, he should undergo an interesting transformation, though you'll probably want to leave the house in case there are any side effects. Then go back and question Sithik and tell")
        npc<Neutral>("him the effects won't wear off until he tells the truth. In fact, that's not exactly true, but I'm sure it'll be an extra incentive to get him to be honest.")
        guildMenu()
    }
