...
class UglugNar : Script {
    init {
        ...
        npcOperate("Trade", "uglug_nar") { (target) ->
            if (get("thzfe_sold_balm", false)) {
                ...
                npc<Neutral>("Me's not got no glug-glugs to sell, yous bring me da sickies glug-glug den me's open da stufsies for ya.")
            }
        }
    ...
    fun ChoiceOption.whatsGoingOn(): Unit = option<Neutral>("Hey, what's going on here?") {
        npc<Neutral>("Dem's dead ogre's come out of da ground...dey's makin' da rest of us into sick-uns ...and dead-uns.")
    ...
    fun ChoiceOption.helloAgain(): Unit = option<Neutral>("Hello again.") {
        if (get("thzfe_sold_balm", false)) {
            npc<Neutral>("Hey yous creature...yous did good fings gedin that glug-glugs for da sickies! All is ogries pepels are not gettin dead cos of you.")
        } else {
            npc<Neutral>("Hey yous creature...yous still here?")
            player<Neutral>("Yeah, I'm going to help Grish by figuring out what went on here.")
            npc<Neutral>("If yous finds somefin for da sickies, yous brings to me...and I's gives you bright pretties, den me make more for alls pepels.")
    ...
    private fun registerSale(potion: String, price: Int) {
        itemOnNPCOperate(potion, "uglug_nar") {
            if (get("thzfe_sold_balm", false)) {
                npc<Neutral>("Yous creatures is da funny ones...yous already solds me's ones now..and us can now sell un to yous!")
                return@itemOnNPCOperate
            }
            item(item = potion, text = "You show the potion to Uglug Nar.")
            player<Neutral>("Hey, here you go! I brought you some of the potion which should cure the disease. You said that you would buy some from me.")
            npc<Neutral>("Yous creatures done da good fing...yous get many bright pretties for dis...!")
