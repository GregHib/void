...
class Grish : Script {
    ...
    init {
        ...
        itemOnNPCOperate("black_prism", "grish") { (target) ->
            ...
            npc<Neutral>(
                "Whas you's shuvvin wizzy stuff in Grish face...is a pretty one but dat's " +
                    "more stuff for da wizzy's dan Grish.",
            )
        }
        ...
        itemOnNPCOperate("torn_page", "grish") { (target) ->
            ...
            player<Neutral>(
                "This torn page was on a lectern in the tomb, do you know why?",
            )
            npc<Neutral>(
                "Dat's der wizzy stuff, not Ogery stuffsies like what Grish got. Das not " +
                    "even big enough for empty da big blower on! No use for Grish dat " +
                    "creatures...you's keeps it.",
            )
        }
        ...
        itemOnNPCOperate("dragon_inn_tankard", "grish") { (target) ->
            ...
            player<Neutral>(
                "I found this tankard in the tomb, have you got any suggestions?",
            )
            npc<Neutral>(
                "Das a good drinker for da drinkies dat un is...is a small-un for Grish so " +
                    "yous creature keeps it yes. Yous creature keeps da fimble drinkers for " +
                    "da smaller drinkies.",
            )
        }
    }
    ...
    private suspend fun Player.intro() {
        ...
        npc<Neutral>(
            "Hey yous creature...wha's you's doing here? Yous be cleverer to be running so " +
                "da sickies from da zogres don't dead ya.",
        )
        introMenu()
    }

    suspend fun Player.introMenu() {
        choice {
            justLookingAround()
            whatDoYouMeanSickies()
            whatAreZogres()
            sorryHaveToGo()
        }
    }

    suspend fun Player.introMenuExpanded() {
        choice {
            justLookingAround()
            whatDoYouMeanSickies()
            whatAreZogres()
    ...
    fun ChoiceOption.justLookingAround(): Unit = option<Neutral>("I'm just looking around thanks.") {
        npc<Neutral>(
            "Yous creature won'ts see muchly in dis place...just da zogries coming wiv da " +
                "sickies.",
        )
        introMenuExpanded()
    }

    fun ChoiceOption.whatDoYouMeanSickies(): Unit = option<Neutral>("What do you mean sickies?") {
        npc<Neutral>(
            "Da zogries comin wiv da sickies...yous get bashed by da zogries and get da " +
                "sickies...den you gonna be like da zogries.",
        )
        player<Neutral>("Sorry, I just don't understand...")
        grish.anim("ogre_fake_death")
        npc<Neutral>(
            "Da sickies is when yous creature goes like orange till green and then goes " +
                "'Urggghhhh!' <col=0000ff>~ Grish imitates falling down with only the white " +
                "of his eyes visible. ~",
        )
        introMenuExpanded()
    }

    fun ChoiceOption.whatAreZogres(): Unit = option<Neutral>("What are Zogres?") {
        npc<Neutral>(
            "Da Zogres are da bigun nasties wiv da sickies, deys old pals of Grish but deys " +
                "jig in Jiggig when dey's full home is deep in da dirt, dey's is not da same " +
                "dead'uns like was before.",
        )
        npc<Neutral>(
            "Dem zogries commin from da under dirt and us is lost for da Jiggie jig place.",
        )
        introMenuExpanded()
    }
    ...
    fun ChoiceOption.canIHelp(): Unit = option<Neutral>("Can I help in any way?") {
        npc<Neutral>(
            "Yes creatures...yous does good fings for Grish and learn why Zogries at Jiggig " +
                "and den get da Zogries back in da ground.",
        )
        player<Neutral>(
            "Oh, so you want me to find out why the Zogres have appeared and then find a way " +
                "of burying them?",
        )
        npc<Neutral>(
            "Is what Grish says! But dis is da biggy danger fing yous creatures...yous be " +
                "geddin' sickies most surely...yous needs be ready..wiv da foodies un da " +
                "glug-glugs.",
        )
        player<Neutral>(
            "Right, so you think there's a good chance that I can get ill from this, so I " +
                "need to get some food and something to drink?",
        )
        npc<Neutral>(
    ...
    fun ChoiceOption.tooDangerousOption(): Unit = option<Neutral>(
        "Hmm, sorry, it sounds a bit too dangerous.",
    ) {
        npc<Neutral>(
            "Yous creature is not a stoopid one...stays out of dere, like clever Grish. Yous " +
                "can paint circles on chest and be da Shaman too!",
        )
        player<Neutral>("Hmm, is it too late to reconsider?")
    }

    fun ChoiceOption.okayCheckThings(): Unit = option<Neutral>(
    ...
    private suspend fun Player.confirmQuestStart() {
        npc<Neutral>(
            "Is yous creatures really, really sure yous wanna do dis creatures..we's got no " +
                "glug-glugs for da sickies? We's knows nuffin for da going of da sickies?",
        )
    ...
    fun ChoiceOption.reallySure(): Unit = option<Neutral>("Yes, I'm really sure!") {
        if (!meetsZogreRequirements()) {
            npc<Mad>(
                "Sorry, yous creatures, but yous is too green behind da ears for dis job " +
                    "Grish finks.",
            )
        ...
        npc<Neutral>(
            "Dats da good fing yous creature...yous does Grish a good fing. But yous know dat " +
                "yous get sickies and mebe get dead!",
        )
        player<Neutral>(
            "If that's your idea of a pep talk, I have to say that it leaves a lot to be " +
                "desired.",
        )
        npc<Neutral>(
    ...
    fun ChoiceOption.foundResponsibleOption(): Unit = option<Neutral>(
        ...
        player<Neutral>(
            "The person responsible is a wizard named 'Sithik Ints' and he's going to be in " +
                "serious trouble. He told me that the spell which raised the zogres from the " +
                "ground will last forever.",
        )
        player<Neutral>(
            "I'm sorry to say, but you'll have to move the site of your ceremonial dancing " +
                "somewhere else.",
        )
        npc<Neutral>(
        ...
        npc<Neutral>(
            "Urghhh...not good fing creature, yous gotta get da ogrish old fings for da " +
                "making new jiggig special. You's creature needs da key for getting in da low " +
                "bury place.",
        )
        ...
        npc<Neutral>(
            "Yeah creature, yous gotta get da ogrish old fings for da making new jiggig and " +
                "proper in da special way.",
        )
    }
    ...
    fun ChoiceOption.cureDiseaseOption(): Unit = option(
        ...
        npc<Neutral>(
            "Dat's da good fing creature, yous do good fing to give un to Uglug...he gives " +
                "bright pretties for da sickies glug glug.",
        )
    ...
    fun ChoiceOption.otherQuestionsOption(): Unit = option<Neutral>(
    ...
    private suspend fun Player.teachCompositeBow() {
        ...
        player<Neutral>(
            "I know how to make large arrows...you know, 'big stabbers', to kill the " +
                "zogres...they're bigger and apparently do a lot of damage, only thing is, " +
                "the normal ogre bow I need to fire it is quite slow.",
        )
        npc<Neutral>(
            "Why you's not say so creature...me's shows you how to make da bigger stabber " +
                "chucker... <blue>~ Grish gets a couple of items out of his back pack.~", // TODO makes too much of the line blue
        )
        set("thzfe_makecompozogrebow", true)
        items(
            "achey_tree_logs",
            "wolf_bones",
            "Grish shows you he has Achey tree logs and wolf bones, he starts to whittle away " +
                "at them both with a knife.",
        )
        item(item = "unstrung_comp_bow", text = "Grish shows you his achievement, a rather powerful looking composite bow frame...")
        items(
            "unstrung_comp_bow",
            "bowstring",
            "He shows you the bow frame and the string and after some time and a great deal " +
                "of effort, he strings the composite ogre bow.",
        )
    ...
    fun ChoiceOption.shamansOption(): Unit = option<Neutral>(
        "Why are you much nicer than the Shaman in Gu'Tanoth?",
    ) {
        npc<Neutral>(
            "Dey's is da big crazy one's! Dey's biggest angries wiv fings and wanna dead all " +
                "fings...dey's gotten da biggies wizzy stuff...and dey's wanna eat yous " +
                "creatures...Grish, not do dat...",
        )
        player<Neutral>(
            "Oh, well that's a relief! It's good to know you don't eat humans...",
        )
        npc<Neutral>(
            "Grish not say dat! Me's want's tasty looking creatures for yums...you's looks " +
                "like da sickies chompy...not good for da gutsies...",
        )
        player<Scared>("Gulp!")
        ...
        npc<Neutral>(
            "But it's da  big laffsies when it's facey goes to whiteness....ha ha ha!",
        )
        // TODO: switch chathead to Uglug Nar
    ...
    fun ChoiceOption.doYouKnowRantz(): Unit = option<Neutral>("Do you know Rantz?") {
        npc<Neutral>(
            "Me's know's about Rantz, he's da biggun chompy hunter..he finks...ha ha ha!",
        )
        player<Neutral>("How do you mean?")
        npc<Neutral>(
            "He's da bad shot chompy sticker, no good at sneaky, sneaky part, he's more " +
                "gooder at da 'noisy, noisy miss da chompy', ha ha ha! ",
        )
        if (zogre_flesh_eaters == 8) otherQuestionsLimited() else otherQuestionsFull()
    }

    fun ChoiceOption.whyDoesntRantzLive(): Unit = option<Neutral>(
        "Why doesn't Rantz live with the rest of the Ogres?",
    ) {
        npc<Neutral>(
            "He's been leaving Gu 'Noth 'cos dey's peoples is da big stressy dere? All da " +
                "ogries is busying all da time...not doin' no good for da healfy fing. Rantz " +
                "is da brave-un tho! He's got da big secret fing for leaving Gu' Noth but me's " +
                "not knowin it. But maybe's he's just want's to be da better chompy sticker?",
        )
        if (zogre_flesh_eaters == 8) otherQuestionsLimited() else otherQuestionsFull()
    }
    ...
    fun ChoiceOption.talkAboutQuestOption(): Unit = option<Neutral>(
    ...
    fun ChoiceOption.easierWay(): Unit = option<Neutral>(
        "There must be an easier way to kill these zogres!",
    ) {
        npc<Neutral>(
            "Yous creature jus makin da bigga stabber chucker like Grish shows you...",
        )
        postNoMenu()
    }

    fun ChoiceOption.cureDisease(): Unit = option<Neutral>(
        "There must be a way to cure this disease!",
    ) {
        npc<Neutral>(
            "Did yous creature makes da sickies glug glug and putin some wiv Uglug for bright " +
                "pretties? He's goodun for makin' da glug glugs...yous maken da glug-glug, " +
                "den sellin' one for Uglug, he's makin' more of da sickies glug",
        )
    ...
    fun ChoiceOption.howIsItGoing(): Unit = option<Neutral>("How's everything going now?") {
        npc<Neutral>(
            "All da zogries stayin' in da oldie Jiggig, we's gonna do da new Jiggig someways " +
                "else. Yous creature da good-un for geddin' da oldie fings...",
        )
        postFinishMenu()
    }
    ...
    fun ChoiceOption.haveThemHere(): Unit = option<Neutral>("Yeah, I have them here!") {
        npc<Happy>(
            "Dat is da goodly fing yous creature, now's we's can make da new Jiggig place " +
                "away from zogries! Yous been da big helpy fing yous creature, Grish wishin' " +
                "yous good stuff for da next fings for creature.",
        )
        npc<Neutral>(
            "<col=000080>~ Grish seems very pleased about the return of the <col=000080>artefacts. ~",
        )
