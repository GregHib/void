...
class CaptainRovin : Script {
    ...
    suspend fun ChoiceBuilder2.palaceGuard(): Unit = option<Quiz>("I am one of the palace guards.") {
    ...
    suspend fun ChoiceBuilder2.whatAboutKing(): Unit = option<Quiz>("What about the King? Surely you'd let him up here.") {
    ...
    suspend fun ChoiceBuilder2.itsImportant(): Unit = option<Talk>(
    ...
    suspend fun ChoiceBuilder2.newRecruit(): Unit = option<Talk>("I'm a new recruit.") {
    ...
    suspend fun ChoiceBuilder2.plasticSurgery(): Unit = option<Talk>("I've had extensive plastic surgery.") {
    ...
    suspend fun ChoiceBuilder2.theresADemon(): Unit = option<Talk>(
    ...
    suspend fun ChoiceBuilder2.forgot(): Unit = option<Upset>("Erm I forgot.") {
        npc<Angry>("Well it can't be that important then.")
        player<Talk>("How do you know?")
        npc<Angry>("Just go away.")
    }

    suspend fun ChoiceBuilder2.aleDelivery(): Unit = option<Neutral>("The castle has just received its ale delivery.") {
    ...
    suspend fun ChoiceBuilder2.notReallyPowerful(): Unit = option<Talk>("Not really.") {
        npc<Happy>("Well, I'm sure the palace guards can deal with it, then. Thanks for the information.")
    }

    suspend fun ChoiceBuilder2.yesVeryPowerful(): Unit = option<Upset>("Yes, very.") {
    ...
    suspend fun ChoiceBuilder2.palaceGuardsAreRubbish(): Unit = option<Chuckle>("Yeah, the palace guards are rubbish!") {
        npc<Chuckle>("Yeah, they're--")
        npc<Angry>("Wait! How dare you insult the palace guards? Get out of my sight!")
    }

    suspend fun ChoiceBuilder2.illFightIt(): Unit = option<Talk>("It's not them who are going to fight the demon, it's me.") {
    ...
    suspend fun ChoiceBuilder2.arisSaidSo(): Unit = option<Talk>("Fortune-teller Aris said I was destined to kill the demon.") {
    ...
    suspend fun ChoiceBuilder2.demonWillDestroyCity(): Unit = option<Afraid>("Otherwise the demon will destroy the city!") {
    ...
    suspend fun ChoiceBuilder2.prysinSaidSo(): Unit = option<Talk>("Sir Prysin said you would give me the key.") {
    ...
    suspend fun ChoiceBuilder2.whyDidHeGiveKeyToYou(): Unit = option<Quiz>("Why did he give you one of the keys then?") {
