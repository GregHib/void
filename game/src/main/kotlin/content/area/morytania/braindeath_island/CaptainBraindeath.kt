...
class CaptainBraindeath : Script {
    ...
    private suspend fun Player.introCutsceneIntro() {
        ...
        npc<Shifty>(
            "When ye arrived ye took a nasty knock to the head, so ye probably don't remember " +
                "agreein' to help us out. But I swear to ye that ye did.",
        )
        ...
        npc<Neutral>(
            "Where it lies is a secret, because ye are standin' in the brewery of Cap'n " +
                "Braindeath, purveyor of the most vitriolic alcoholic beverages in the world!",
        )
        player<Neutral>("Wow!")
        npc<Happy>(
        ...
        npc<Neutral>(
            "With my crew of sturdy, upright pirate brewers, we sail the seven seas, " +
                "distributing cheap 'alcohol' to all and sundry.",
        )
        ...
        npc<Neutral>(
            "Day and night they seek to break our will with their chantin', and their singin' " +
                "and their passive resistance!",
        )
        npc<Neutral>(
            "Seems they lost their fightin' spirit after the first few days. Now most of them " +
                "just protest all the time.",
        )
        player<Quiz>("So, what do you want me to do?")
        npc<Neutral>(
            "Well, me and the lads got our heads together and decided that if we can get their " +
                "Cap'n drunk enough, perhaps they'll stop protestin'.",
        )
        npc<Neutral>(
        ...
        npc<Neutral>(
            "The only problem is that those rottin' fiends have torn up and destroyed all but " +
                "one of the Blindweed Patches.",
        )
        if (hasSpace()) {
            addOrDrop("blindweed_seed")
            set("rum_deal", "given_seeds")
            npc<Neutral>(
                "Here, ${ladOrLass()}. I'll give ye the seed you'll need fer growin' the herb. " +
                    "Help yerself to the gardenin' equipment in the basement.",
            )
            blindweedWarnings()
        } else {
            set("rum_deal", "met_braindeath")
            npc<Neutral>(
                "Well, I have some Blindweed seeds fer ye. When ye have some free space fer " +
    ...
    private suspend fun Player.blindweedWarnings() {
        ...
        npc<Neutral>(
            "Try hecklin' 'em from a distance. Those Swabs may talk a good fight, but if ye " +
                "can put a scare in 'em they'll keep out of yer way!",
        )
    }
    ...
    private suspend fun Player.handoutBlindweedSeed() {
        if (hasSpace()) {
            ...
            set("rum_deal", "given_seeds")
            npc<Happy>(
                "Finally, ${ladOrLass()}! Here, take this seed and grow us some Blindweed.",
            )
            npc<Neutral>(
                "Ye can help yerself to the gardenin' equipment in the basement, just be quick " +
                    "about it!",
            )
            blindweedWarnings()
        } else {
    ...
    private suspend fun Player.deliverBlindweed() {
        if (inventory.contains("blindweed")) {
            ...
            if (hasSpace()) {
                ...
                npc<Neutral>(
                    "When ye've quite finished blubbin', come see me and I'll give ye some Blindweed.",
                )
                npc<Neutral>(
                    "Clear a little space from yer pack first though, ${ladOrLass()}, or ye'll " +
                        "be hard pressed to hold it.",
                )
            }
        }
    }
    ...
    private suspend fun Player.nextStagnantWater() {
        ...
        npc<Neutral>(
            "Well, now ye've shoved the Blindweed into the mix, what we need is a bucket of " +
                "stagnant water.",
        )
        player<Quiz>("Where can I get some of that?")
        npc<Happy>(
            "Ye won't have to go far, ${ladOrLass()}, we have a pool of the stuff here!",
        )
        ...
        npc<Neutral>(
            "All ye have to do is get past the keen-eyed lookout that's been spottin' my men " +
                "when I send 'em.",
        )
    ...
    private suspend fun Player.nextSluglings() {
        ...
        npc<Neutral>(
            "No, ${ladOrLass()}, next ye'll need to go outside and catch five loads of " +
                "Sluglings fer the brew.",
        )
        ...
        npc<Shifty>(
            "Yer not too susceptible to mind control are ye ${ladOrLass()}?",
        )
        player<Quiz>("Why?")
        npc<Shifty>(
        ...
        if (isSeaSlugComplete()) {
            ...
            npc<Angry>(
                "Aye they're related! Tis a good job they'll starve if they tried te eat yer " +
                    "brains!",
            )
        } else {
        ...
        if (hasSpace()) {
            ...
            npc<Neutral>(
                "Just dunk it in the water. I'm sure a clever ${ladOrLass()} like yerself will " +
                    "have no problem.",
            )
            npc<Neutral>(
                "Oh, and if ye haul up some squiddy-looking things, don't tate to shove 'em in " +
                    "the barrel too.",
            )
            npc<Neutral>("They add a special, fishy texture to the drink.")
        } else {
            npc<Neutral>(
                "Ye'll be needin' a fishbowl in a big net to help ye catch them slimy little " +
                    "blighters.",
            )
            set("rum_deal", "catch_creatures")
    ...
    private suspend fun Player.slugShockSnap() {
        ...
        npc<Neutral>(
            "No problem. Well, now ye have just one more ingredient to grab, and then we can " +
                "get this 'rum' flowin!",
        )
        ...
        npc<Neutral>(
            "Hold yer horses, ${ladOrLass()}! While ye was off gallivantin' with yer slimy " +
                "aquatic playmates, the 'rum' achieved spiritual critical mass.",
        )
        ...
        npc<Neutral>(
            "Well, to tell the truth, my lads are a little quicker off the mark, so it only " +
        ...
        if (hasSpace()) {
            ...
            npc<Neutral>(
                "One of the lads did a little priestin' on the side before he came here. Get " +
                    "him to bless it and ye'll do fine.",
            )
        } else {
            set("rum_deal", "bless_wrench")
            npc<Happy>(
                "We've got a special item fer dealin' with this problem. Dump some of that " +
                    "stuff yer carryin' and I'll hand it over!",
            )
        }
    }
    ...
    private suspend fun Player.finalIngredient() {
        npc<Neutral>(
            "Well, now that ye've got that spirit out of there ye can dump in the final " +
                "ingredient.",
        )
        ...
        npc<Neutral>(
            "When yer quite done flappin' yer lips, go down into the basement and whack " +
                "spiders until ye find a fever spider body.",
        )
        npc<Neutral>("Shove it in the hopper, and then we're in business.")
        when {
            wearingSlayerGloves() -> {
                npc<Happy>(
                    "I see yer already wearin' some Slayer Gloves. That'll keep the Fever " +
                        "Spiders from gnawin' yer hands off!",
                )
            }
            inventory.contains("slayer_gloves") -> {
                npc<Neutral>(
                    "Ye'll be wanting to put them Slayer Gloves of yours on before ye head off " +
                        "${ladOrLass()}, as those Fever Spiders carry a nasty disease.",
                )
                npc<Neutral>(
                    "They'll give it to ye if ye aren't wearin' somethin' too thick fer them " +
                        "to bite through.",
                )
            }
            else -> {
                ...
                npc<Neutral>(
                    "These Fever Spiders carry a nasty disease, and ye'll need a pair of " +
                        "Slayer Gloves on to not catch it!",
                )
            }
        }
    ...
    private suspend fun Player.spiderInHopper() {
        ...
        npc<Neutral>(
            "Try givin' it to the Captain, he's in charge. Get him bladdered and the rest " +
                "will fall!",
        )
        when {
            inventory.contains("bucket") -> {
                npc<Neutral>(
                    "Ye'll need to use that bucket of yours.  Most stuff can't stand bein' in " +
                        "contact with our 'rum' fer too long.",
                )
            ...
            hasSpace() -> {
                ...
                npc<Neutral>(
                    "Here, ye'll need to use one of these.  The rum tends to eat through " +
                ...
                npc<Neutral>(
                    "Oh, and don't try using anything but a bucket. Our 'rum' tends to eat " +
                        "through stuff.",
                )
            }
        }
    ...
    private suspend fun Player.spokeToCaptainYes() {
        ...
        npc<Neutral>(
            "With those rottin' dogs legless they'll never keep fighting us now, so we've " +
                "decided to stay here and keep the 'rum' flowin!",
        )
    ...
    private suspend fun Player.offerRum() {
        npc<Quiz>(
            "Fancy some 'rum', ${ladOrLass()}? It's still fresh. Well, fresh-ish...",
        )
    ...
    fun ChoiceOption.whatDoYouWantMeToDo(): Unit = option<Neutral>(
        "What exactly do you want me to do?",
    ) {
        when (questStage("rum_deal")) {
            3 -> {
                npc<Neutral>(
                    "Well, I need ye to go grow me some Blindweed, but ye are carryin' too " +
                        "much stuff fer me to give ye the seed.",
                )
            }
    ...
    fun ChoiceOption.whyTalkLikePirate(): Unit = option("Why do you talk like a pirate?") {
        ...
        npc<Neutral>(
            "However, 'tis also true that I stumbled across the basic recipe fer my most " +
                "potent of brews in a terrible alchemical accident.",
        )
        npc<Neutral>(
            "See, 'twas a dark and stormy night, and the wind was howlin' around the trees as " +
                "I worked late into the night.",
        )
        npc<Neutral>(
            "Steppin' too close to a candle with my flask in my hand, I was suddenly swept up " +
                "in a terrible, yet potently alcoholic, explosion.",
        )
        player<Quiz>("And?")
        npc<Happy>(
            "Well' ${ladOrLass()}, it seems the fumes from that first batch of me 'rum' did " +
                "strange things to me brain.",
        )
        npc<Neutral>(
            "I don't remember the exact words the healers used, but apparently the stuff " +
                "burned out the tiny, specialised part of me brain that tells me not to talk " +
                "like a pirate.",
        )
    ...
    fun ChoiceOption.whatDoYouMakeHere(): Unit = option<Neutral>("So what do you make here anyway?") {
        ...
        npc<Neutral>(
            "The finest, most potent, most flammable and most debilitatin' 'rum' in the whole " +
                "of RuneScape!",
        )
        ...
        npc<Neutral>(
            "If we called the stuff we make 'rum' without makin' the little quote gestures " +
                "every time, then the Cookin' Guild has promised to do entertaininly painful " +
                "things to us with whisks.",
        )
        npc<Neutral>("See, technically -")
        npc<Shifty>(
            "And by that I mean technically according to the Disposal of Hazardous Waste Act " +
                "and the Health and Safety Laws -",
        )
        npc<Neutral>(
            "technically, what we're brewin' here is Artificially Produced Hyper Condensed " +
                "Sweetened 'Rum' Flavour Distillate.",
        )
        player<Quiz>("Riiiiiiiight...")
        npc<Neutral>(
            "So ye see, ${ladOrLass()}, we just call it 'rum' because the real name be a bit " +
                "of a mouthful.",
        )
    ...
    private suspend fun Player.growBlindweedReminder() {
        ...
        npc<Neutral>(
            "Best be careful, ${ladOrLass()}, fer them pirates will skin ye alive if they " +
                "catch ye.",
        )
        when {
            ...
            inventory.contains("blindweed_seed") -> {
                ...
                npc<Angry>(
                    "Then I'll have ye flogged, hung, slapped with a haddock and sent back " +
                        "out there to fight the zombie hordes with little more than harsh language!",
                )
                if (hasSpace()) {
                    ...
                    npc<Neutral>(
                        "Come back and see me when ye ain't loaded down with stuff, and I'll " +
                            "give ye another.",
                    )
                }
            }
    ...
    private suspend fun Player.putBlindweedInHopper() {
        ...
        if (inventory.contains("blindweed")) return
        player<Quiz>(
            "And what happens if I lose the Blindweed between here and the intake hopper?",
        )
        player<Shifty>(
            "Theoretically, of course, as it would be almost impossible for me to simply lose " +
                "the stuff in the time it will take me.",
        )
        npc<Angry>("Gah!")
        npc<Neutral>(
            "Yer nothin' better than a sea-slug wearin', lilly-legged, bandy-livered " +
                "landlubber!",
        )
        if (hasSpace()) {
            npc<Neutral>(
                "Here, I was savin' a little fer an emergency, so take this and shove it in " +
                    "the hopper before I bung ye in there and turn the handle!",
            )
            addOrDrop("blindweed")
            player<Neutral>("You had some all the time?")
            player<Neutral>(
                "Why was I risking my neck killing pirates to grow some if you had some all " +
                    "along?",
            )
            npc<Neutral>("Less talk, ${ladOrLass()}, haven't ye got a hopper to be fillin'?")
        } else {
            npc<Neutral>(
                "I've got a little put aside fer an emergency. Drop some of that junk yer " +
                    "carryin', and come see me to get it.",
            )
        }
    }
    ...
    private suspend fun Player.pourBucketInHopper() {
        ...
        if (inventory.contains("bucket_of_water_stagnant")) return
        // Stolen-bucket scene
        ...
        if (hasSpace()) {
            ...
            npc<Neutral>(
                "Take this bucket and pour it into the hopper. If ye don't get it right this " +
                    "time I'll have yer guts fer garters!",
            )
        } else {
            npc<Neutral>(
                "Well, 'tis a good job I saved a little, isn't it? Now get rid of something " +
                    "so I can give it to ye.",
            )
        }
    }
    ...
    private suspend fun Player.catchFiveSeaCreatures() {
        npc<Neutral>(
            "I want ye to go outside and catch five sea creatures from the squid fishin' spot.",
        )
        npc<Neutral>(
            "When ye've grabbed them, jam 'em in the barrel in the attic and pressurise 'em.",
        )
        if (inventory.contains("fishbowl_and_net")) return
        ...
        when {
            !inventory.contains("fishbowl") && inventory.contains("big_fishing_net") -> {
                if (hasSpace()) {
                    ...
                    npc<Angry>(
                        "Yer lucky all my precious fish died last week, ${ladOrLass()}. Take this and " +
                            "wrap it in that net, then go grab some Sluglings!",
                    )
                } else {
                    npc<Angry>(
                        "Well, I'll give ye a fishbowl fer that net of yours if ye'll drop " +
                            "some of that clutter in yer pack.",
                    )
                }
            }
            !inventory.contains("big_fishing_net") && inventory.contains("fishbowl") -> {
                ...
                if (hasSpace()) {
                    ...
                    npc<Neutral>(
                        "Here, wrap this around yer fishbowl and get back out there!",
                    )
                } else {
                    npc<Neutral>(
                        "Well, I'll give ye a net fer that fishbowl of yours if ye'll drop " +
                            "some of that clutter in yer pack.",
                    )
                }
            }
                ...
                if (hasSpace()) {
                    npc<Neutral>(
                        "Here! Try not ter lose this one! These things don't grow on trees, " +
                            "ye know.",
                    )
                    inventory.add("fishbowl_and_net")
                } else {
                    npc<Neutral>(
                        "Come see me when yer not so laden down. I've got another one fer ye " +
                            "if ye need it.",
                    )
                }
            }
    ...
    private suspend fun Player.clearEvilSpirit() {
        npc<Neutral>(
            "I want ye to go clear the Evil Spirit out of the brewin' controls. Use the " +
                "wrench I gave ye, but get it blessed first.",
        )
        when {
            inventory.contains("holy_wrench") -> {
                npc<Neutral>(
                    "I see ye've got the wrench good and blessed, so go whack the controls " +
                        "till the Evil Spirit pops out.",
                )
            }
            inventory.contains("wrench") -> {
                ...
                if (hasSpace()) {
                    ...
                    npc<Neutral>(
                        "If ye lose this one I'll have ye turned inside out, covered with " +
                            "spiders and turned rightside in so they'll eat ye alive!",
                    )
                    ...
                    npc<Neutral>(
                        "Look, I've got another wrench, but ye don't have any free space fer " +
                            "it. Come back when ye do.",
                    )
                }
            }
        }
