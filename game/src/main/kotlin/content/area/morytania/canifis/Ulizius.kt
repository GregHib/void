...
class Ulizius : Script {
    init {
        ...
        objectOperate("Open", "gate_mort_myre*_closed") { (target) ->
            ...
            if (quest("nature_spirit") == "unstarted") {
                statement("There's a message attached to this gate, it reads:<br><navy> ~ Mort Myre is a dangerous Ghast infested swamp. ~<br><navy> ~ Do not enter if you value your life. ~<br><navy> ~ All persons wishing to enter must see Drezel. ~")
                return@objectOperate
            }
    ...
    private suspend fun Player.canIGoThrough() {
        ...
        npc<Scared>("Absolutely not. I've been given strict instructions not to let anyone through. It's just too dangerous. No one gets in without Drezel's say so!")
        if (quest("nature_spirit") == "unstarted") {
            player<Quiz>("Where is Drezel?")
            npc<Scared>("Oh, he's in the temple, just go back over the bridge, down the ladder and along the hallway, you can't miss him.")
        } else {
